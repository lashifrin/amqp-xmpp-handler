...
start_link(Args) ->
    ?DBG("=====> start link in dispatcher_ep ~n",[]),
    ...
    case proplists:get_value(channel, Args) of
        Channel when is_pid(Channel) ->
            ...
            Tag = amqp_channel:subscribe(
                    ...
                    self()),
            ?DBG("subscribe to a  queue: ~p~n", [Tag]),
            ?DBG("Starting to listern on Queue ~p~n ", [Queue]),
